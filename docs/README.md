...
2. `STORY_WORLD.md`
   - 세계관, 스토리 구조, 주제
3. `MISSION_FACTIONS.md`
   - 의뢰 세력, 관계축, 초반 서사 구조
4. `PROJECT_REDESIGN_STEPS.md`
   - 침투 런 방향으로의 재설계 진행 로그 (Steps 1-7 부분 구현 완료)
5. `MULTI_WINDOW_EXPANSION.md`
   - 멀티윈도우 확장 설계 (Clipboard Window 1차 구현 완료)
6. `class_diagram.puml`
...
- 역할: `res/mission/` 코드 구조와 세력 설계(`MISSION_FACTIONS.md`) 간 매핑; 갭 분석 및 다음 데이터 진화 계획.
- 상태: **코드/설계 정합 참고 문서**
...
- 상태: **아카이브 포인터**
- 비고: 핵심 내용은 `PROJECT_REDESIGN_STEPS.md`에 흡수됨
...
- 코어 비전 변경 -> `GAME_CONCEPT.md` 업데이트
- 세계관/서사 변경 -> `STORY_WORLD.md` 업데이트
- 세력/의뢰 구조 변경 -> `MISSION_FACTIONS.md` 업데이트
- 구현 진행/다음 단계 변경 -> `PROJECT_REDESIGN_STEPS.md` 업데이트
- 특정 기능의 깊은 설계 -> 별도 문서 추가
